...
import { getDebugHint, reset as resetAI } from '../features/ai/aiSlice';
...
export default function SubmissionDetailView({ submission }) {
    ...
    const { hint, isFetchingHint } = useSelector(state => state.ai);
    ...
    const handleGetHint = () => {
        dispatch(getDebugHint(submission._id)).then(action => {
        ...
        <div className="p-4 overflow-y-auto h-full space-y-6">
            ...
            {submission.verdict === 'Wrong Answer' && submission.failedTestCase && (
                <div className="bg-secondary p-4 rounded-lg space-y-3 border border-border-color">
                    ...
                    <div className="pt-2">
                        {!hint && (
                            <button onClick={handleGetHint} disabled={isFetchingHint} className="mt-2 w-full inline-flex items-center justify-center gap-2 bg-indigo-600 text-white font-medium py-2 px-4 rounded-md hover:bg-indigo-500 disabled:bg-slate-500 transition-colors">
